...
- 队长：生成、复制、撤销邀请码，按人邀请，改成员角色，移除成员
...
队内角色只有两种：**Team Member**（手册里叫队员）和 **Team Owner**（队长）。一个队可以有多个队长，但**不能一个都没有**——最后一名队长不能降级、不能被移除、也不能离开，得先把别人提上来。
...
兑码失败时，不管是码打错、过期、次数用完还是被队长撤销，提示都是同一句 `Invalid or expired invite code`，界面不会告诉你是哪一种。连续兑错会被暂时挡住：`Too many invalid invite attempts. Try again later.`

### 被队长按名字邀请，要你同意

这条路必须你点头才算数。队长发出邀请后，你在**铃铛 → Inbox** 里会看到一条 **Team invite**，正文是 `{谁} invited you to {队伍名} as Team Member`，点 **Accept** 或 **Decline**。处理过的行会显示 **Accepted** / **Declined**，没处理的显示 **Pending**。

**Accept 之前你不属于这个队**，队里的项目一个也看不见。

## 队长：邀请码

**Teams** 页选中团队，**Invite codes** 卡片（只有队长看得到）→ **New**：
...
**Invite** 那一栏（只有队长看得到），在 `Search name, username, or email` 里输入至少 2 个字符，从候选里选人，点 **Invite**，弹 **Invite sent**。
...
- 这条路发出去的角色固定是 **Team Member**，界面上没有角色可选。想让人当队长，等他进队之后在成员表里点 **Make owner**。
- 已经在队里的人再被邀请会失败，角色不变。
...
- **Make owner**：提成队长，弹 **Promoted to Team Owner**。
- **Make member**：降成队员，弹 **Changed to Team member**。
...
| 你想做     | 被拦的原因       | 界面上写的                                                                                                                                       |
| ------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| 把自己降成队员 | 你是最后一名队长    | `A team must keep at least one Team Owner`                                                                                                  |
| 移除某个成员  | 他是最后一名队长    | `A team must keep at least one Team Owner`                                                                                                  |
| 移除某个成员  | 他是某些项目唯一的管家 | `Appoint another Project Owner for {名字} first`                                                                                              |
| 离开团队    | 你是最后一名队长    | `Promote another member to Team Owner first`                                                                                                |
| 离开团队    | 你是某些项目唯一的管家 | `Appoint another Project Owner for {项目} first`；如果队里还没有别人可以接手，则是 `Invite someone to this team and appoint them Project Owner for {项目} first` |
| 离开团队    | 你是队里唯一成员    | `You are the only member — delete the team instead`                                                                                         |
| 删团队     | 你不是队长       | `Only a Team Owner can delete a team`                                                                                                       |
...
团队详情的 **Projects** 卡片里，每个项目行上有一个 **Owners** 入口，能看和改这个项目的管家名单：**Add Team Member** 选人，点 **Add**，弹 **Project Owner added** / **Project Owner removed**。一个管家都没有时显示 `No Project Owners. Ask an Admin to appoint one.`
...
| 你看到                                                  | 原因                                                              |
| ---------------------------------------------------- | --------------------------------------------------------------- |
| `Invalid or expired invite code`                     | 码打错、过期、次数用完，或被队长撤销了——四种情况提示都一样，找队长重新生成一个                        |
