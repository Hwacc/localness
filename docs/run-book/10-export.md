...
**词条**：下面是一张带勾选的词条表，筛选方式和词条表那一页一样（`Search key or text`、状态、日期、**Show __draft_ keys**）。表里有一列显示文案，列头是国旗 + 源语言简称（比如 **Eng**）——那就是这条词条的原文。两个标记值得认：
...
**没有单独的原文列**——源语言那一列就是原文，不重复放一遍。一条词条在同一页上有几个框，就有几行，文案相同。
